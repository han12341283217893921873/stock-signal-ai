...
console.log(`📝 API Key: ${FINNHUB_API_KEY.substring(0, 8)}...${FINNHUB_API_KEY.slice(-4)}`);
...
async function testUSStockQuote() {
  ...
  try {
    ...
    console.log(`   타임스탬프: ${new Date(response.data.t * 1000).toISOString()}`);
...
async function testHistoricalData() {
  ...
  try {
    ...
    if (response.data.c && response.data.c.length > 0) {
      ...
      console.log(`   최신 캔들 (날짜: ${new Date(response.data.t[latest] * 1000).toISOString().split('T')[0]}):`);
...
async function runAllTests() {
  ...
  const tests = [
    ...
    { name: "주식 검색", result: results.search },
