...
export const users = sqliteTable("users", {
  ...
  cashBalance: real("cashBalance").default(100000).notNull(), // 시작 자본금 $100,000
  krwBalance: real("krwBalance").default(30000000).notNull(), // 시작 자본금 ₩30,000,000
  realizedPnl: real("realizedPnl").default(0).notNull(), // USD 실현 손익
  realizedPnlKrw: real("realizedPnlKrw").default(0).notNull(), // KRW 실현 손익
...
export const portfolioPositions = sqliteTable(
  "portfolio_positions",
  {
    ...
    memo: text("memo"),
    entrySignalScore: integer("entrySignalScore"), // 진입 시점 AI 점수
    addedAt: integer("addedAt", { mode: "timestamp" })
      .default(sql`(strftime('%s', 'now'))`)
      .notNull(),
    updatedAt: integer("updatedAt", { mode: "timestamp" })
...
// scanHistory: market enum에 "all" 추가
